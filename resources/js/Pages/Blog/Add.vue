    ...
    <AuthenticatedLayout>
        <template #header>
            ...
            <form @submit.prevent="form.post('/blog/add')" class="mt-5">
                ...
                <quill-editor v-if="tampilan" v-model:value="form.content" />
                <div v-else>
                    <textarea class="bg-gray-800 w-full text-gray-300 font-mono text-sm leading-5 p-2" v-model="form.content"></textarea>
                </div>
                <!-- <input type="text" v-model="form.judul">
                <div v-if="form.errors.judul">{{ form.errors.judul }}</div> -->
                <br>

                <Button @click="submit" class="mt-5" title="Simpan">
